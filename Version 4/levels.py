...
def level_over():
    ...
    draw_text(game_play_screen, "You have completed level {}".format(level), 40, CENTRE_X, HEIGHT / 4)    # Shows the completed level to the user
    draw_text(game_play_screen, "Your score is {}".format(score), 40, CENTRE_X, CENTRE_Y)    # Shows the current score to the user
    draw_text(game_play_screen, "Press y to play the next level", 30, CENTRE_X, HEIGHT * 3/4)    # Asks the user if they want to carry on playing
    pygame.display.flip()
    waiting = True
    while waiting:
        game_clock.tick(FPS)
...
def show_scores():
    draw_text(game_play_screen, "Score: {}".format(score), 22, 53, 300)    # Shows the scores to the user
    draw_text(game_play_screen, "Bricks Remaining: {}".format(len(all_game_bricks)), 22, 100, 330)    # Shows the remaining bricks to the user
#####################################
...
background = pygame.image.load(path.join(image_folder, "moon.png")).convert()
...
def new_game():
    ...
    game_ball.centerx = random.randint(50, WIDTH - 50)
    game_ball.rect.centery = random.randint(CENTRE_Y, HEIGHT - 300)
